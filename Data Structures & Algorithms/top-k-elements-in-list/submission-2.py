class Solution:
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        # The max frequency is at most length of the input
        ...
        k_freq = []
        while n>0 and k>0:
            if bucket_freq[n]: # the list is empty or not
                take_ele = min(k,len(bucket_freq[n]))
                k_freq.extend(bucket_freq[n][:take_ele])
                k -= take_ele
            n-=1
        return k_freq
